...
def parse_argv():
    ...
    parser.add_argument("--new_source", metavar='bloomberg|reuters',
        type=str, default='bloomberg', help="News source")

    parser.add_argument("--src", metavar='URI',
        type=str, default='~/Download/20061020_20131126_bloomberg_news',
    ...
    if not (args.start >= MIN_DATE[args.new_source] and \
        ...
        fatal('date range must be from %s to %s for %s data' % \
            (MIN_DATE[args.new_source].strftime('%Y-%m'),
             MAX_DATE[args.new_source].strftime('%Y-%m'),
             args.new_source))
...
class RawReader(io.IOBase):
    ...
    START_DATE = datetime.datetime(2006,10,20)
    ...
    def output(self, fin, date):
        if self.opts.dst.startswith('file://'):
            ...
            try:
                ...
                if error_code == 404:
                    fatal("%s does not exists" % self.opts.dst[5:])
